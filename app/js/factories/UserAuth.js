function UserAuth(UserResourceService, $cookies, $log, $q) {
  "ngInject";
      ...
      return $q(resolve => {
        if (userSlug || this._getUserFromStorage()) {
          ...
          this._authenticateUser(userSlug)
            .catch(error => {
              $log.log(
                `login:: login failed with slug ${userSlug}, creating new user`
              );
              this._createNewUser().then(newUser => {
                Factory.currentUser = newUser;
                $log.log(
                  `login:: created new user and logged in as: ${
                    Factory.currentUser.name
                  }`
                );
    ...
    _authenticateUser: (userSlug = null) => {
      ...
      return $q((resolve, reject) => {
        ...
        UserResourceService.get(
          { userSlug: userSlug ? userSlug : userCookie.random_slugs[0] },
          function(response) {
