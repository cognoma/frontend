function factoryWrapper($log, _, $q, $timeout, $http, AppSettings) {
  ...
  DiseaseModel.prototype._loadSamples = function() {
    let _model = this;
    const SAMPLES_ENDPOINT = `${API_BASE}${AppSettings.api.samples}?disease=${
      this.acronym
    }`;

    $log.log(`_loadSamples:${SAMPLES_ENDPOINT}`);

    return $http.get(SAMPLES_ENDPOINT).then(samplesResponse => {
      _model.samples = samplesResponse.data.count;
      $log.log(
        `_loadSamples:${samplesResponse.data.count} loaded for ${this.acronym}`
      );
  ...
  DiseaseModel.prototype._loadMutatedGenes = function(mutationsGenes = []) {
    ...
    const POSITIVES_ENDPOINT = `${API_BASE}${
      AppSettings.api.samples
    }?limit=1&disease=${this.acronym}${mutationsParams}`;

    if (mutationsGenes.length) {
      return $http.get(POSITIVES_ENDPOINT).then(mutatedGenesResponse => {
